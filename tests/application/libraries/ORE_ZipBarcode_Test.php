...
use ore\ORE_ZipBarcode;
use ore\tests\oreTestCase;

require_once dirname(dirname(dirname(__FILE__))).'/oreTestCase.php';

/**
 * Class ORE_ZipBarcode_Test
 *
 * @package ore\tests\application\libraries
 */
class ORE_ZipBarcode_Test extends oreTestCase {
	...
	public function test_test() {
		...
		foreach ($tests as $test) {
			$zipbar = new ORE_ZipBarcode($test['zip'], $test['addr'], false);
			$code_cc_comp = implode('', $zipbar->code_cc_comp());
			$ans = preg_replace('[\s]', '', $test['ans']);
			$this->assertSame($code_cc_comp, $ans);
		}
